...
class L2TP(Screen):
    pass
class L2TP_client(Screen):
    pass
class SSTP_client(Screen):
    pass
class Static(Screen):
    pass
class PPPoE(Screen):
...
class MikroLiteApp(MDApp):
    ...
    def autoispv6(self):
        try:
            res = self.api.get_resource('/ipv6/dhcp-client')
            index = str(res.get()).find("id")
            if index > 1:
                res = self.api.get_resource('/ipv6/dhcp-client')
                index = str(res.get()).find("id")
                ...
                index = str(res.get()).find("id")
                if index < 1:
                    res.add(interface='ether1', request='address,prefix', add_default_route='yes', pool_name='dhcp', disabled='no')
        except:
            pass
    def IPv6(self):
        try:
            ...
            index = str(res.get()).find("id")
            if index > 1:
                index = str(res.get()).find("disabled=no")
                self.root.get_screen('IPv6').ids.isp_dhcpv6_button.text = 'Выкл'
                index = str(res.get()).find("status")
                a = index + 10
                while a < len(str(res.get())):
                    if str(res.get())[a] == "'":
                        break
                        ...
                        a = a + 1
                self.root.get_screen('IPv6').ids.status_ipv6_dhcp.text = 'Статус: '+str(res.get())[index + 10:a]
            ##HEv6
            res = self.api.get_resource('/interface')
            index = str(res.get()).find("Hurricane Electric IPv6 Tunnel Broker")
            if index > 1:
                self.root.get_screen('HEv6').ids.status_ipv6_HEv6.text = 'Статус: Активно'
        except:
            pass
    def HEv6(self):#################################################################################################################################
        try:
            script = self.root.get_screen('HEv6').ids.HE_script.text
            res = self.api.get_resource('/system/script')
            res.add(name='MikroLite_HE',
                    source=';/system script remove MikroLite_HE')
            index = str(res.get()).find("MikroLite_HE")
            if index > 1:
                script_to_run = res.get(name="MikroLite_firewall_del")[0]
                self.api.get_binary_resource('/').call('system/script/run', {"number": script_to_run["id"].encode("utf-8")})
            else:
                res.add(name='MikroLite_HE',
                        source=':foreach id in [/ip firewall filter find] do {/ip firewall filter remove $id};/system script remove MikroLite_firewall_del')
                script_to_run = res.get(name="MikroLite_HE")[0]
                self.api.get_binary_resource('/').call('system/script/run', {"number": script_to_run["id"].encode("utf-8")})
        except:
            pass
    ...
    def resour(self):
        ...
        try:
            ...
            pass
    def nichego(self):
        self.root.get_screen('SSTP').ids.prosto_button.text = 'Выкл'
    def nichego1(self):
        self.root.get_screen('L2TP').ids.prosto_button1.text = 'Выкл'
    def nichego2(self):
        self.root.get_screen('L2TP_client').ids.prosto_button2.text = 'Выкл'
    def nichego3(self):
        self.root.get_screen('SSTP_client').ids.prosto_button3.text = 'Выкл'
    def build(self):
        self.sm = Manager()
        ...
        self.sm.add_widget(SSTP(name='SSTP'))
        self.sm.add_widget(L2TP(name='L2TP'))
        self.sm.add_widget(L2TP_client(name='L2TP_client'))
        self.sm.add_widget(SSTP_client(name='SSTP_client'))
        self.sm.add_widget(IPv6(name='IPv6'))
        self.sm.add_widget(Staticv6(name='Staticv6'))
        self.sm.add_widget(Docker(name='Docker'))
        self.sm.current = 'Authorization'
